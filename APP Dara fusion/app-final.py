...
def calibrate_camera(object_points, image_points, K, dist_coeffs=None, use_ransac=False):
    ...
    for flag in flags_to_try:
        try:
            if use_ransac:
                # SolvePnPRansac para filtrar outliers (puntos mal medidos/marcados)
            ...
            # Capturar y continuar al siguiente si hay un fallo interno
            continue
...
defaults = {
    ...
    "stl_units": "mm",
    "stl_file_name": "",
    "camera_profiles": {"Cámara A (Principal)": 1.0, "Cámara B (Secundaria)": 1.0},
...
def load_uploaded_image(file_bytes):
    """
    Lee un archivo de imagen en bytes, aplica la orientación correcta de EXIF (exif_transpose),
    y retorna la imagen PIL y el array numpy RGB.
    """
...
def get_scaled_K(K, target_shape, source_shape):
    """
    Escala la matriz K si las dimensiones de la imagen destino (target_shape: (W_target, H_target))
    difieren de las dimensiones de calibración (source_shape: (W_source, H_source)).
    """
    if K is None:
        return None
    ...
    K_scaled = K.copy()
    K_scaled[0, 0] *= w_scale  # fx
    K_scaled[1, 1] *= h_scale  # fy
    K_scaled[0, 2] *= w_scale  # cx
    K_scaled[1, 2] *= h_scale  # cy
    return K_scaled
...
if page == "📷 0. Parámetros de Cámara":
    ...
    with col_cb1:
        cb_cols = st.number_input("Esquinas internas — columnas", min_value=3, max_value=20, value=8,
                                   help="Número de esquinas internas horizontales del tablero (no cuadrados).")
        cb_rows = st.number_input("Esquinas internas — filas", min_value=3, max_value=20, value=5,
                                   help="Número de esquinas internas verticales del tablero.")
    with col_cb2:
        square_size = st.number_input("Tamaño de cuadrado (mm)", min_value=1.0, max_value=200.0, value=30.0, step=0.5,
        ...
        if st.button("🔬 Calibrar con Tablero (cv2.calibrateCamera)", use_container_width=True):
            with st.spinner("Detectando esquinas y calibrando..."):
                ...
                for i, f in enumerate(chessboard_files):
                    raw = f.read()
                    pil_cb, np_cb = load_uploaded_image(raw)
                ...
                if found_count < 4:
                    ...
                    st.session_state.calibration_shape = img_shape
...
# ══════════════════════════════════════════
    ...
    with col_stl:
        ...
        if st.session_state.vertices is not None:
            ...
            name_stl = st.session_state.stl_file_name
            st.success(f"✅ `{name_stl}` | {len(f_arr)} caras | {len(v)} vértices")
            if st.button("🗑️ Quitar modelo", key="btn_clear_stl", use_container_width=True):
                st.session_state.vertices = None
                st.session_state.faces = None
                st.session_state.stl_file_name = ""
                st.rerun()
    ...
    with col_photos:
        ...
        if st.session_state.images:
            ...
            if st.button("🗑️ Quitar todas las fotos", key="btn_clear_photos", use_container_width=True):
    ...
    if st.session_state.vertices is not None:
        ...
        fig = go.Figure(data=[go.Mesh3d(
            x=v[:,0] + st.session_state.offset_x, y=v[:,1] + st.session_state.offset_y, z=v[:,2] + st.session_state.offset_z,
...
# ══════════════════════════════════════════
    ...
    if not st.session_state.images:
        ...
        # 1. Aplicar undistort si tenemos K calibrada (en el tamaño original para evitar que quede negro)
        is_undistorted = False
        if st.session_state.K_matrix is not None and st.session_state.dist_coeffs is not None:
            # Obtener K escalada para la resolución original de esta foto
            scaled_K = get_scaled_K(
                st.session_state.K_matrix,
                target_shape=(orig_w, orig_h),
                source_shape=st.session_state.calibration_shape
            )
            np_orig = np.array(pil_img)
            np_disp = cv2.undistort(np_orig, scaled_K, st.session_state.dist_coeffs)
            pil_img_for_display = Image.fromarray(np_disp)
            is_undistorted = True
        else:
            pil_img_for_display = pil_img

        # 2. Redimensionar para visualización
        max_display_width = 1000
        if orig_w > max_display_width:
            scale_factor = max_display_width / orig_w
            cache_key_resize = f"resized_{image_name}_undist_{is_undistorted}"
            if cache_key_resize not in st.session_state:
                st.session_state[cache_key_resize] = pil_img_for_display.resize(
...
# ══════════════════════════════════════════
    ...
    if st.session_state.vertices is None:
        ...
        if not ready_images:
            ...
            # --- NUEVA SECCIÓN DE ROBUSTEZ Y OFFSET DE CALIBRACIÓN ---
            st.markdown("#### 🛠️ Configuración de Robustez y Ajuste de Malla")
            
            col_ransac, col_offset = st.columns([1, 1])
            with col_ransac:
                st.markdown("##### 🔬 Algoritmo de Resección")
                st.session_state.use_ransac = st.checkbox(
                    "Usar RANSAC (Filtro de outliers)",
                    value=st.session_state.use_ransac,
                    help="Si está activado, utiliza cv2.solvePnPRansac para ignorar puntos fiduciarios que tengan grandes errores de medición física o marcado accidental."
                )
                
            with col_offset:
                st.markdown("##### 📏 Corrección de Offset 3D (Origen STL)")
                st.caption("Ajustá el offset para alinear el origen del STL con el origen físico de medición con calibre.")
                col_offx, col_offy, col_offz = st.columns(3)
                with col_offx:
                    st.session_state.offset_x = st.number_input("Offset X", value=st.session_state.offset_x, step=1.0, format="%.2f")
                with col_offy:
                    st.session_state.offset_y = st.number_input("Offset Y", value=st.session_state.offset_y, step=1.0, format="%.2f")
                with col_offz:
                    st.session_state.offset_z = st.number_input("Offset Z", value=st.session_state.offset_z, step=1.0, format="%.2f")
            ...
            if st.button("🚀 Calibrar poses de cámaras", use_container_width=True):
                with st.spinner("Resolviendo SolvePnP..."):
                    ...
                    for img_name in ready_images:
                        ...
                        # Aplicar Offset 3D
                        obj_pts[:, 0] += st.session_state.offset_x
                        obj_pts[:, 1] += st.session_state.offset_y
                        obj_pts[:, 2] += st.session_state.offset_z
                        ...
                        dist_coefs = st.session_state.dist_coeffs if st.session_state.K_source == "chessboard" else None
                        rvec, tvec, C, pitch, roll, yaw, success = calibrate_camera(
                            obj_pts, img_pts, K,
                            dist_coeffs=dist_coefs,
                            use_ransac=st.session_state.use_ransac
                        )
                        if success:
                ...
                try:
                    ...
                    for img in st.session_state.images:
                        ...
                        if (st.session_state.K_matrix is not None and
                            ...
                            arr = cv2.undistort(arr, scaled_K, st.session_state.dist_coeffs)
                        images_dict[img["name"]] = arr

                    # Aplicar offset a los vértices antes de proyectar para que coincida con las calibraciones de cámara
                    v_offsetted = v.copy()
                    v_offsetted[:, 0] += st.session_state.offset_x
                    v_offsetted[:, 1] += st.session_state.offset_y
                    v_offsetted[:, 2] += st.session_state.offset_z

                    colors = backward_projection(v_offsetted, f_arr, images_dict, st.session_state.calibrations)
