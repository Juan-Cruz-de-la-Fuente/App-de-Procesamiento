...
def show_data_fusion():
                ...
                if sel_proj != "-- Seleccionar --" and sel_proj != "-- Seleccionar Proyecto --":
                    if st.session_state.df_project_name != sel_proj:
                        ...
                        with st.spinner("Descargando proyecto completo..."):
                            ...
                            # Cargar Video (Descarga y Extracción Local Al Vuelo)
                            new_images = []
                            video_file = next((f for f in files if f['name'].lower().endswith('.mp4')), None)
                            if video_file:
                                with st.spinner("Descargando video desde Drive y extrayendo frames localmente..."):
                                    raw_vid = drive_api.download_file(video_file['id'])
                                    if raw_vid:
                                        import tempfile
                                        import os
                                        ...
                                        target_fps = cfg.get("extraction_fps", 5) # Default 5 fps
                                        ...
                                        while True:
                                            ...
                                            if count % interval == 0:
                                                ...
                                                # Camera assignment (Default to Camera 1 for Video V1)
                                                cam_asignada = "Cámara 1"
                                                new_images.append({
                                                    "name": f"frame_{frame_idx:04d}",
                                                    "pil": pil_img,
                                                    "np": frame_rgb,
                                                    "raw": None, # Do not keep raw in memory
                                                    "camera": cam_asignada
                                                })
                                                frame_idx += 1
                                            count += 1
        ...
        if st.session_state.df_project_id:
            ...
            if c_save.button("💾 GUARDAR CAMBIOS EN GOOGLE DRIVE", use_container_width=True, type="primary"):
                with st.spinner("Guardando archivos y configuraciones en tu Drive..."):
                    ...
                    # Guardar archivo de video subido previamente
                    if hasattr(st.session_state, "raw_video_bytes") and st.session_state.raw_video_bytes:
                        drive_api.upload_file(st.session_state.raw_video_bytes, "ensayo_video.mp4", st.session_state.df_project_id, mimetype='video/mp4')
                        st.session_state.raw_video_bytes = None # Free memory
    ...
    # ═══════════════════════════════════════════════════════════════
    elif sub_page == "🎬 1. Carga de Video":
        st.markdown("<div class='section-card'>", unsafe_allow_html=True)
        st.subheader("🎬 Video del Ensayo (Oil Flow)")
        
        c_up, c_fps = st.columns([3, 1])
        with c_up:
            uploaded_video = st.file_uploader("Subir Video (.mp4)", type=["mp4", "avi", "mov"])
        with c_fps:
            extraction_fps = st.number_input("Extraer FPS", min_value=1, max_value=30, value=5, help="Cuadros por segundo a extraer del video.")
            
        if uploaded_video:
            if st.button("Procesar Video", use_container_width=True, type="primary"):
                with st.spinner("Extrayendo frames del video en memoria local..."):
                    import tempfile
                    import os
                    from PIL import Image
                    st.session_state.raw_video_bytes = uploaded_video.getvalue()
                    
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as tmp:
                        tmp.write(st.session_state.raw_video_bytes)
                        tmp_path = tmp.name
                    
                    cap = cv2.VideoCapture(tmp_path)
                    fps = cap.get(cv2.CAP_PROP_FPS) or 30
                    interval = max(1, int(fps / extraction_fps))
                    ...
                    while True:
                        ...
                        if count % interval == 0:
                            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                            pil_img = Image.fromarray(frame_rgb)
                            st.session_state.df_images.append({
                                "name": f"frame_{frame_idx:04d}",
                                "pil": pil_img,
                                "np": frame_rgb,
                                "raw": None,
                    ...
                    cap.release()
                    os.remove(tmp_path)
                st.success(f"Video procesado: {len(st.session_state.df_images)} cuadros extraídos. (No olvides Guardar en Drive para respaldar el archivo .mp4)")
                
        if st.session_state.df_images:
            st.info(f"Se extraerán y mapearán {len(st.session_state.df_images)} instantes de tiempo del video.")
            # Mostrar slider para previsualizar frames
            if len(st.session_state.df_images) > 0:
                frame_idx = st.slider("Previsualizar Frame", 0, len(st.session_state.df_images)-1, 0)
                st.image(st.session_state.df_images[frame_idx]['pil'], caption=f"Instante temporal {frame_idx}", use_column_width=True)
        ...
        if st.button("🎨 INICIAR TEXTURIZADO DEL MODELO", use_container_width=True, type="primary"):
            with st.spinner("Ejecutando mapeo vectorizado..."):
                try:
                    ...
                    st.success(f"✅ Texturizado completado con éxito en modo OilFlow!")
                except Exception as e:
